...
trait EventEmitterTrait {
	...
	public function emittingCall(\Closure $fn, $arguments ,$class, $eventName) {
		\OC::$server->getEventDispatcher()->addListener("$class.before$eventName.data", function (GenericEvent $event) use (&$arguments, &$class, &$eventName) {
			\OC::$server->getEventDispatcher()->dispatch("$class.before$eventName", new GenericEvent(null, $event->getArguments()));
		});
		//Collect data from the after event
		\OC::$server->getEventDispatcher()->addListener("$class.after$eventName.data", function (GenericEvent $event) use (&$arguments) {
			$arguments['after'] = $event->getArguments();
		});
		...
		$result = $fn();
		if (isset($arguments['after']) and (count($arguments['after']) > 0)) {
