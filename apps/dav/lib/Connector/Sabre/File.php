		...
		return $this->emittingCall(function () use (&$data) {
			...
			try {
				...
				if ($view) {
					...
					$event->setArgument('path', $this->path);
					\OC::$server->getEventDispatcher()->dispatch('sabrefile.beforecreate.data', $event);
				} else {
					$run = true;
				}
				...
				if ($view) {
					...
					$event->setArgument('fileinfo', $view->getFileInfo($this->path));
					\OC::$server->getEventDispatcher()->dispatch('sabrefile.aftercreate.data', $event);
				}
